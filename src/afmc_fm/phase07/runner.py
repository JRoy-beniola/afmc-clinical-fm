...
from afmc_fm.phase07.diagnostics import first_patience_exhaustion_epoch
...
class Phase07CellRun:
    ...
    would_patience_exhaust_epoch: int | None


def prepare_phase07_cohort(
...
def run_phase07_cell(
    ...
    if cell.n_train not in phase05_config.train_sizes:
        raise ValueError("Phase 0.7 cell n_train is absent from the Phase 0.5 config")
    ...
    fit_phase05_model(
        ...
        stop_on_patience=cell.optimization_policy == "standard_early_stop",
    )

    trace = recorder.trace_frame()
    patience_event = first_patience_exhaustion_epoch(
        trace,
        patience=phase05_config.patience,
    )
    summary: dict[str, object] = dict(recorder.summary_payload())
    summary["optimization_policy"] = cell.optimization_policy
    summary["would_patience_exhaust_epoch"] = patience_event
    ...
    return Phase07CellRun(
        ...
        optimization_policy=cell.optimization_policy,
        would_patience_exhaust_epoch=patience_event,
    )


__all__ = ["Phase07CellRun", "prepare_phase07_cohort", "run_phase07_cell"]
